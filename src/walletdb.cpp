...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call Bulpcoin with fAggressive=true to
    // get as much data as possible.
    // Rewrite bulpcoind data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> bulpcoindData;
    bool allOK = dbenv.Bulpcoin(newFilename, true, bulpcoindData);
    if (bulpcoindData.empty()) {
        LogPrintf("Bulpcoin(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("Bulpcoin(aggressive) found %u records\n", bulpcoindData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, bulpcoindData) {
        if (fOnlyKeys) {
