...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BULP)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount bulpAvailableBalance = balance - immatureBalance - nLockedBalance;

    // BULP Watch-Only Balance
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
    CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

    // BULP labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, bulpAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // BULP Available
    bool showBULPAvailable = settingShowAllBalances || bulpAvailableBalance != nTotalBalance;
    bool showWatchOnlyBULPAvailable = showBULPAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showBULPAvailable || showWatchOnlyBULPAvailable);
    ui->labelBalance->setVisible(showBULPAvailable || showWatchOnlyBULPAvailable);
    ui->labelWatchAvailable->setVisible(showBULPAvailable && showWatchOnly);

    // BULP Pending
    bool showBULPPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyBULPPending = showBULPPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showBULPPending || showWatchOnlyBULPPending);
    ui->labelUnconfirmed->setVisible(showBULPPending || showWatchOnlyBULPPending);
    ui->labelWatchPending->setVisible(showBULPPending && showWatchOnly);
    ...
    // BULP Locked
    bool showBULPLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyBULPLocked = showBULPLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showBULPLocked || showWatchOnlyBULPLocked);
    ui->labelLockedBalance->setVisible(showBULPLocked || showWatchOnlyBULPLocked);
    ui->labelWatchLocked->setVisible(showBULPLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BULP")
    updateDisplayUnit();
}
